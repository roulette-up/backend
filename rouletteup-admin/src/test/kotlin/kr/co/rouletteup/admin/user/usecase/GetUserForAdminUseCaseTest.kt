...
class GetUserForAdminUseCaseTest {
    ...
    inner class GetUsers {
        ...
        @Test
        fun `soft delete 포함 사용자 목록을 페이징 조회하고 AdminUserSummary로 변환한다`() {
            // given
            ...
            every { userService.readAllIncludingDeleted(pageable) } returns page
            ...
            verify(exactly = 1) { userService.readAllIncludingDeleted(pageable) }
    ...
    inner class GetUserById {
        ...
        fun `사용자가 존재하면 AdminUserDetail로 변환해 반환한다`() {
            ...
            every { userService.readByIdIncludeDeleted(userId) } returns user
            ...
            verify(exactly = 1) { userService.readByIdIncludeDeleted(userId) }
        ...
        fun `사용자가 없으면 NOT_FOUND 예외를 던진다`() {
            // given
            val userId = 999L
            every { userService.readByIdIncludeDeleted(userId) } returns null
